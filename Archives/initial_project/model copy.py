...
# Combine X and y into one for filtering
train = X_train.copy()
...
# Downsample negative class to match size of positive class * k
neg_downsampled = resample(neg, replace=False, n_samples=len(pos)*5, random_state=42)

# Combine
trimmed = pd.concat([pos, neg_downsampled])
X_trim = trimmed.drop('target', axis=1)
y_trim = trimmed['target']
